...
export default function SemanticSimilaritySplit() {
  ...
  return (
    <div className="container mx-auto p-6 space-y-8 max-w-6xl">
      ...
      >
        ...
        <p className="text-gray-600">使用语义相似度进行智能切分，可自定义切分参数</p>
      </motion.div>
        ...
        <Card className="p-6 bg-white shadow-lg hover:shadow-xl transition-shadow duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              >
                {BREAKPOINT_TYPE_OPTIONS.map(opt => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
              </select>
            </div>
            <div className="space-y-2">
              <Label className="text-sm font-medium text-gray-700">断点阈值 (breakpoint_threshold_amount)</Label>
              <Input
                type="number"
                value={breakpointAmount}
                onChange={e => setBreakpointAmount(Number(e.target.value))}
                min={0}
                max={1}
                step={0.01}
                disabled={isLoading}
            ...
            <div className="flex items-center justify-center w-full">
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="text-xs text-gray-500">支持 TXT, Markdown</p>
                </div>
                <Input
                  type="file"
                  accept=".txt,.md"
